...
function create_attatchment($local,$html,$lab){
		...
		$fp = "/var/www/fish/assets/scheduled_reports/tmp/" . $lab . "_Monthly_Report.doc"	; 
		$handle = fopen($fp, 'w+');
...
function output_titles($local,$doctype, $column_array){ 	  
	if ($doctype =="l_summary"){
		...
		if ($column_array == "mutant" || $column_array == "strain" || $column_array == "transgene"){
			...
			$column_array[1]['db_name'] = "starting_adults";
			...
			$column_array = "";
			$column_array[0]['db_name'] = "starting_adults";
...
function output_custom_fields($temp_object, $doctype, $location, $local, $index){	
	...
	if ($temp_object["starting_adults"] == "" || $temp_object["starting_adults"] == "0"){ 
		$temp_object["starting_adults"] = '<a href="#" style=" visibility:hidden">$' . 0 . '</a>';					 								 
	}	
	...
	if ($doctype =="l_summary"){			 
			...
			$html .= '<td>' . $temp_object['starting_adults'] . '</td>';
...
function output_qsummary_rows($local,$listby,$type){
	...
	$html = "<br>";
		...
		if(strstr($listby,'_lab')){
			...
			$criteria = "user.name like '" . $listby . "'";
		}
		if($type == "user"){		 			
			$sql = "select count(batch_ID) as total_batches, sum(starting_adults) as starting_adults,sum(current_adults) as current_adults,sum(starting_nursery) as starting_nursery from fish, user where fish.user_ID = user.user_ID and " . $criteria;			 
		}elseif ($type == "mutant"){			 
			$sql = "select distinct fish.mutant_ID,mutant.*, count(batch_ID) as total_batches, sum(starting_adults) as starting_adults,sum(current_adults) as current_adults,sum(starting_nursery) as starting_nursery from fish, user,mutant where mutant.mutant_ID = fish.mutant_ID and fish.user_ID = user.user_ID and " . $criteria . " group by fish.mutant_ID";
		}elseif ($type == "strain"){
			$sql = "select distinct fish.strain_ID,strain.*, count(batch_ID) as total_batches, sum(starting_adults) as starting_adults,sum(current_adults) as current_adults,sum(starting_nursery) as starting_nursery from fish, user,strain where strain.strain_ID = fish.strain_ID and fish.user_ID = user.user_ID and " . $criteria . " group by fish.strain_ID";
		}elseif ($type == "transgene"){
			$sql = "select distinct fish.transgene_ID,transgene.*, count(batch_ID) as total_batches, sum(starting_adults) as starting_adults,sum(current_adults) as current_adults,sum(starting_nursery) as starting_nursery from fish, user,transgene where transgene.transgene_ID = fish.transgene_ID and fish.user_ID = user.user_ID and " . $criteria . " group by fish.transgene_ID";
		}		 
...
while($lab_object =  mysql_fetch_assoc($outer_Recordset)){	
	$query_Recordset = "select * from user where lab like '" . $lab_object['lab'] . "'";
	...
	while($lab_object =  mysql_fetch_assoc($Recordset)){  
		$fileatt = "/var/www/fish/assets/scheduled_reports/tmp/"; // Path to the file
		...
		$email_txt .= '<br><br><a href="https://aragorn.bio.purdue.edu/tmp/' . $lab . '_Monthly_Report.doc">Click Here</a> to view your report.';
