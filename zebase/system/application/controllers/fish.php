...
class fish extends Controller { 
	...
	function index(){    
		...
		$this->db->join('fish FS', 'FS.batch_ID = STAT.batch_ID','left outer');
		$this->db->join('users', 'FS.user_ID = users.user_ID','left outer');
		$this->db->join('labs', 'users.lab = labs.lab_ID','left outer');
		...
		$end_date = mktime(1,1,1,date("m",time()),31,date("Y",time()));
